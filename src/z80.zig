...
pub const z80 = struct {
    ...
    pub const flag_bits = packed struct { c: u1, n: u1, pv: u1, __: u1, h: u1, _: u1, z: u1, s: u1 };
    ...
    fn rotate(self: *z80, instruction: u8, ram: []u8, direction: u8, thru_carry: bool) void {
        ...
        switch (direction) {
            'L' => {
                ...
                value = (value << 1) | (if (thru_carry) carry else 0); // Rotate left
            },
            'R' => {
                self.f.bits.c = if (value & 0x01 != 0) 1 else 0; // Set carry flag with lsb
                value = (value >> 1) | (if (thru_carry) carry << 7 else 0); // Rotate right
            },
    ...
    fn misc_instructions(self: *z80, ram: []u8) void {
        ...
        switch (opcode) {
            ...
            .SBC_HL_BC, .SBC_HL_DE, .SBC_HL_HL, .SBC_HL_SP => {
                ...
                self.set_register_pair(.HL, result);
                self.f.bits.c = if (result > 0xFFFF) 1 else 0; // Set carry if underflow
            ...
            .LD_BC__NN_, .LD_DE__NN_, .LD_HL__NN_, .LD_SP__NN_ => {
                ...
                self.pc += 2; // Increment PC by 1 to account for the low byte of the address
            ...
            .LD_I_A => self.i = self.a,
            .LD_A_I => self.a = self.i,
            .LD_R_A => self.r = self.a,
            .LD_A_R => self.a = self.r,
