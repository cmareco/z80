...
// Data structure for holding test data
// Matches the json structure of the test files
pub const TestData = struct {
    name: []const u8,
    initial: Initial,
    final: Final,
    cycles: []const struct { u16, ?u8, []const u8 },
    //ports: ?[]const struct { u16, u8, []const u8 },
};
...
pub fn run_test(test_data: TestData) bool {
    ...
    cpu.set_register_pair(.AF_, test_data.initial.af_);
    cpu.set_register_pair(.BC_, test_data.initial.bc_);
    cpu.set_register_pair(.DE_, test_data.initial.de_);
    cpu.set_register_pair(.HL_, test_data.initial.hl_);
    ...
    if (cpu.f.byte & 0b11010111 != test_data.final.f & 0b11010111) {
        fail_test = true;
        std.debug.print("   F mismatch: initial {b:08}, got {b:08}, expected {b:08}\n", .{ test_data.initial.f & 0b11010111, cpu.f.byte & 0b11010111, test_data.final.f & 0b11010111 });
    }
...
pub fn run_test_file(file_name: []const u8) !bool {
    ...
    return if (tests_failed == 0) true else false;
}
...
pub fn run_all_tests(directory: []const u8) !void {
    ...
    for (file_list.items, 0..) |file, idx| {
        ...
        std.debug.print("{}: {s}\n", .{ idx, file[0..] });
        const failed: bool = try run_test_file(full_path);
        if (failed == false) {
            break;
        }
    }
...
pub fn main() !void {
    ...
    if (args.next()) |arg| {
        const full_path = try std.fs.path.join(allocator, &.{ "C:/Users/cmare/Documents/zig/z80-main-tests/z80-main/v1", arg });

        // Run a single test file
        _ = try run_test_file(full_path);
    } else {
        // Run all tests in the default directory
        try run_all_tests("C:/Users/cmare/Documents/zig/z80-main-tests/z80-main/v1");
    }
}
